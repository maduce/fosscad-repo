...
The striker of the impact fuse is intended to be fitted with a thin, paper rupture disk at the base, filled with black powder, and then topped with the screw-in nipple.  The nipple should self thread in the plastic.  The nipple would then be capped with the appropriate sized cap, dropped into the nose tube, and the cover (which incorporates the hammer) snapped into place.  THIS SHOULD NOT BE DONE UNTIL IMMEDIATELY BEFORE FIRING AS THIS ARMS THE DEVICE.  Version 0.2 will include a cross pin safety that locks the impact fuse into place until removed.
